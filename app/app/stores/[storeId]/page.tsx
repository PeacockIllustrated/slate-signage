        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <Link href="/app" className="text-sm text-gray-500 hover:text-gray-900 mb-1 block">&larr; Dashboard</Link>
                    <h1 className="text-2xl font-bold text-gray-900">{store.name}</h1>
                    <p className="text-sm text-gray-500">{store.timezone}</p>
                </div>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {screenSets?.map((set) => (
                    <Link href={`/app/screen-sets/${set.id}`} key={set.id} className="block group">
                        <div className="relative bg-white border border-gray-200 rounded-lg shadow-sm group-hover:border-black transition-colors overflow-hidden">
                            <div className="p-6">
                            ...
                            <div className="bg-gray-50 px-6 py-3 border-t border-gray-100 flex items-center justify-between">
                                <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">Visualise</span>
                                <span className="text-gray-400 group-hover:text-black transition-colors">&rarr;</span>
                            </div>
                        </div>
                    </Link>
                ))}
